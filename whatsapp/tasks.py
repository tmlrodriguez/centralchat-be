...
def process_whatsapp_webhook_task(self, payload):
    """
        DOCSTRING: Process WhatsApp Webhook Task

        Description:
        - Process a previously authenticated global Dialoqo Meta webhook payload in the background.
        - Resolve company ownership from the Meta Phone Number ID contained in each webhook event.
        ...
        - Only the validated JSON payload is passed to Celery.
    ...
    logger.info("Processing global WhatsApp webhook task.")
    result = persist_whatsapp_webhook_payload(payload=payload)
    logger.info("Global WhatsApp webhook task completed: %s", result)
